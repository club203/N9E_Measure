...
func NewSNMPClientByConnectionData(ctx context.Context, ipAddress string, data *SNMPConnectionData) (SNMPClient, error) {
	...
	if data.DiscoverParallelRequests == nil || data.DiscoverTimeout == nil || data.DiscoverRetries == nil || *data.DiscoverParallelRequests <= 0 || *data.DiscoverTimeout <= 0 {
		return nil, utility.NewPreConditionError("invalid connection preferences")
	}
	...
	for i := 0; i < amount; i++ {
		res := <-out
		if res.err != nil {
			if !utility.IsNetworkError(res.err) {
	...
	return nil, utility.NewSNMPError("cannot connect with any of the given connection data")
}
...
func newSNMPClientTestConnection(client *gosnmp.GoSNMP) (*snmpClient, error) {
	...
	if err != nil {
		return nil, utility.NewSNMPError(err.Error())
	}
...
func (s *snmpClient) SNMPGet(ctx context.Context, oid ...OID) ([]SNMPResponse, error) {
	...
	if !successful {
		return nil, utility.NewNotFoundError("No Such Object available on this agent at this OID")
	}
...
func (s *snmpClient) SNMPWalk(ctx context.Context, oid OID) ([]SNMPResponse, error) {
	...
	if response == nil {
		...
		err = utility.NewNotFoundError("No Such Object available on this agent at this OID")
		if s.useCache {
			s.walkCache.add(oid.String(), nil, err)
		}
		return nil, err
...
func (s *SNMPResponse) GetValue() (utility.Value, error) {
	if !s.WasSuccessful() {
		return nil, utility.NewNotFoundError("no such object")
	}
	v, err := s.getValueDecoded()
	if err != nil {
		return nil, errors.Wrap(err, "failed to decode value")
	}
	return utility.New(v), nil
}

// GetValueRaw returns the raw value string of the snmp response.
func (s *SNMPResponse) GetValueRaw() (utility.Value, error) {
	if !s.WasSuccessful() {
		return nil, utility.NewNotFoundError("no such object")
	}
	if s.snmpType == gosnmp.OctetString {
		...
		case string:
			return utility.New(strings.ToUpper(hex.EncodeToString([]byte(x)))), nil
		case []byte:
			return utility.New(strings.ToUpper(hex.EncodeToString(x))), nil
		}
	}
	return utility.New(s.value), nil
}
...
func (s *SNMPResponse) GetValueBySNMPGetConfiguration(snmpGetConfig SNMPGetConfiguration) (utility.Value, error) {
	var val utility.Value
	var err error
