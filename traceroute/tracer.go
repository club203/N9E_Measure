...
func (t *Tracer) ServeData(from net.IP, b []byte) error {
	...
	if msg.Type == ipv4.ICMPTypeEchoReply {
		...
		// from就是148.153.93.9，只是TTL过大。那么Trace的目标IP就是148.153.93.9，但这条路径中出现了其他IP。
...
func (t *Tracer) removeSession(s *Session) {
	t.mu.Lock()
	defer t.mu.Unlock()
	// 一个IP只会有一个session，所以直接删除
	delete(t.sess, string(s.ip))
...
func (s *Session) handle(res *packet) {
	...
	n := 0
	var req *packet
	// 找到对应的Request请求包，并且将这个包从probes中移除
	s.mu.Lock()
	for _, r := range s.probes {
	...
	//// 收发包的时间戳
	//logger.Info("send and recv timestamp (ns)", zap.String("Dest IP", req.IP.String()),
	//	zap.String("Response IP", res.IP.String()), zap.Int("TTL", hops),
	//	zap.Int64("Send", req.Time.UnixNano()), zap.Int64("Recv", res.Time.UnixNano()))
	select {
	case s.ch <- &Reply{
		IP:   res.IP,
...
func TraceWithDefaultTTL(ip net.IP) ([]*Hop, error) {
	return Trace(ip, DefaultConfig)
}

// Trace is a simple traceroute tool using DefaultTracer.
func Trace(ip net.IP, cfg Config) ([]*Hop, error) {
	DefaultConfig.MaxHops = cfg.MaxHops
	hops := make([]*Hop, 0, DefaultTracer.MaxHops)
	// 回调函数，找到跳数为dist的存储结果，如果没有就新建
